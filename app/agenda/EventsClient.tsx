...
export function EventsClient({ initialEvents, initialDate }: EventsClientProps) {
  ...
  // Generate 7 days starting from yesterday
  const generateDays = () => {
    const days = [];
    for (let i = -1; i <= 5; i++) {
  ...
  const events = format(selectedDate, 'yyyy-MM-dd') === format(initialDate, 'yyyy-MM-dd') 
      ...
      {error && (
        <div className="bg-white rounded-large shadow-medium">
          <div className="text-center py-8 p-6">
            <svg className="mx-auto mb-4 w-12 h-12 text-danger" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {events.length > 0 ? (
            ...
            <div className="bg-white rounded-large shadow-medium">
              <div className="text-center py-12 p-6">
                <svg className="mx-auto mb-4 w-16 h-16 text-default-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 002 2z" />
                </svg>
                <h3 className="text-xl font-semibold text-default-600 mb-2">No hay eventos</h3>
