...
export async function GET(request: NextRequest) {
  try {
    ...
    const filteredEvents =
      events?.filter((event) => {
        return event.event_schedules?.some(
          (schedule: {
            start_date: string | number | Date;
            recurrence_pattern: string;
            days_of_week: string | any[];
          }) => {
    ...
    const sortedEvents = filteredEvents.sort((a, b) => {
      const aEarliestDate = Math.min(
        ...a.event_schedules.map((s: { start_date: string | number | Date }) =>
          new Date(s.start_date).getTime()
        )
      );
      const bEarliestDate = Math.min(
        ...b.event_schedules.map((s: { start_date: string | number | Date }) =>
          new Date(s.start_date).getTime()
        )
      );
